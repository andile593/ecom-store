...
exports.updateProduct = asyncErrorHandler(async (req, res, next) => {
    ...
    if (req.body.images !== undefined) {
        ...
        for (let i = 0; i < product.images.length; i++) {
            await admin.storage().bucket().file(product.images[i].public_id).delete();
        }
        ...
        for (let i = 0; i < images.length; i++) {
            const file = admin.storage().bucket().file(`products/${Date.now()}_${i}`);
    ...
    if (req.body.logo.length > 0) {
        // Delete existing logo from Firebase Storage
        await admin.storage().bucket().file(product.brand.logo.public_id).delete();

        // Upload new logo to Firebase Storage
        const file = admin.storage().bucket().file(`brands/${Date.now()}_logo`);
        await file.save(req.body.logo, {
            contentType: 'image/jpeg', // Adjust the content type as per your requirement
        });

        const brandLogoUrl = `https://storage.googleapis.com/${file.bucket.name}/${file.name}`;
